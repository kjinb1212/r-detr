    ...
    def __call__(self, bbox_pred, gt_bboxes):
        ...
        Args:
            ...
            gt_rbboxes (Tensor): Ground truth boxes with normalized
                coordinates (x1, y1, w, h, a). Shape [num_gt, 5].
...
class PolyIoUCost:
    ...
    def __call__(self, bboxes, gt_bboxes):
        ...
        Args:
            bboxes (Tensor): Predicted boxes with unnormalized coordinates
                If box_format='xywha' (cx,cy,w,h,a) shape (num_query, 5).
                If box_format='xywha' (x1,y1,x2,y2,x3,y3,x4,y4) shape (num_query, 8).
            gt_bboxes (Tensor): Ground truth boxes with unnormalized
                coordinates.
                If box_format='xywha' (cx,cy,w,h,a) shape (num_query, 5).
                If box_format='xywha' (x1,y1,x2,y2,x3,y3,x4,y4) shape (num_query, 8).
        ...
        Returns:
            torch.Tensor: iou_cost value with weight
        """
        # print('#'*10, 'mmrotate/core/bbox/match_costs/match_cost.py IoUCost', '#'*10)

        # overlaps: [num_bboxes, num_gt]
        overlaps = rbbox_overlaps(
            bboxes, gt_bboxes, mode=self.iou_mode, is_aligned=False) + self.eps
